...
Reference payloads for scripts and agents. Field semantics mirror the
[Emailable API](https://emailable.com/docs/api/emails); optional fields are
omitted when empty.

#### `emailable verify <email> --json`
...
`state` is one of `deliverable`, `undeliverable`, `risky`, `unknown`. `score`
is 0–100. `duration` is seconds.

#### `emailable batch verify ... --json`

Submit response (no `--wait`):

```json
{
  "id": "5cfcbfdeede34200693c4319",
  "message": "Your batch has been accepted and is being processed."
}
```

With `--wait`, the submit step is followed by the same payload as
`batch get --json` below.

#### `emailable batch get <id> --json`

The API returns one of four shapes depending on batch size and state. The
CLI passes them through unchanged.

In progress (no `--partial`):
...
Partial snapshot (`--partial`, batches ≤ 1,000):

```json
{
  "id": "5cfcbfdeede34200693c4319",
  "message": "Batch is still processing; partial results returned.",
  "total_counts": { "total": 1000, "processed": 314 },
  "emails": [
    { "email": "a@example.com", "state": "deliverable", "...": "..." }
  ]
}
```

Completed small batch (≤ 1,000):

```json
{
  "id": "5cfcbfdeede34200693c4319",
  "status": "complete",
  "reason_counts": { "accepted_email": 812, "invalid_domain": 188 },
  "emails": [
    { "email": "a@example.com", "state": "deliverable", "...": "..." }
  ]
}
...
Each entry in `emails` has the same shape as the single-`verify` payload
above.

Completed large batch (> 1,000):

```json
{
  "id": "5cfcbfdeede34200693c4319",
  "status": "complete",
  "download_file": "https://api.emailable.com/v1/batch/.../download"
}
```

Fetch `download_file` to retrieve the full results as CSV.
